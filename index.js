$(window).load(function(){
	...
	setTimeout(function(){
		$(".waitUntilLoaded").animate({"opacity": "1"}, 1000);
	}, 500);
	$("#invalidEmail").css({"width": $("#emailText").width()+$("#submit").width()+10});
	$("#emailText").bind("focus", function(){
		$("#invalidEmail").animate({"opacity": "0.0"},150);
	});
	...
	$(".learnMore").bind("click", function(){
		$("html, body").animate({"scrollTop": $(".infoPage").offset().top+1}, 1000);
	});
...
var addEmail = function(){
	...
	if (atpos< 1 || dotpos<atpos+2 || dotpos+2>=x.length) {
		$("#invalidEmail").animate({"opacity": "0.9"},150);
	}
	else{
		$("#submit").val("Submitting...").attr("onclick", "");
		/*
		 * This code will be used when we have a server
		 * to post requests on to.
		 * 
		$.ajax({
